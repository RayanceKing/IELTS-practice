...
/// Lazily import old UI-owned settings records once. This runs before every
/// topic operation so existing installations keep their data, but the legacy
/// rows are never consulted after the durable marker is written.
pub fn ensure_legacy_writing_topics_imported(conn: &Connection) -> DbResult<()> {
    ...
    let mut stmt = conn.prepare(
        "SELECT key, value_json FROM settings WHERE namespace = ?1 ORDER BY key ASC",
    )?;
    ...
    let marker = json!({ "imported": imported, "skipped": skipped }).to_string();
    tx.execute(
...
pub fn list_writing_topics(
    ...
    let total: i64 = conn.query_row(
        ...
        params![task_type, category.as_deref(), difficulty, search.as_deref()],
        |row| row.get(0),
    )?;
...
fn upsert_writing_topic_inner(
    ...
    let existing_activity: Option<String> = conn
        .query_row(
            "SELECT activity FROM practice_assets WHERE id = ?1",
            params![id],
            |row| row.get(0),
        )
        .optional()?;
    if existing_activity.as_deref().is_some_and(|activity| activity != "writing") {
        return Err(DbError::Validation(format!(
...
fn normalize_writing_topic_command(
    ...
    Ok(NormalizedTopicInput {
        ...
        title_json,
        title_text,
        image_path: normalized_optional_text(command.image_path.as_deref()),
    })
}
...
fn raw_topic_into_dto(raw: RawTopic) -> DbResult<WritingTopicDto> {
    let task_type = WritingTaskType::parse_loose(&raw.task_type).ok_or_else(|| {
        DbError::Validation(format!("stored writing topic has invalid task type: {}", raw.id))
    })?;
    let difficulty = raw
        .difficulty
        .parse::<u8>()
        ...
        .filter(|value| (1..=5).contains(value))
        .ok_or_else(|| {
            DbError::Validation(format!("stored writing topic has invalid difficulty: {}", raw.id))
        })?;
    Ok(WritingTopicDto {
        id: raw.id,
        task_type,
...
fn legacy_topic_candidates(key: &str, value: Value) -> Vec<(Option<String>, Value)> {
    ...
    match value {
        Value::Array(values) => values.into_iter().map(|value| (None, value)).collect(),
